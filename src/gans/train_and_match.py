...
from src.gans.discriminators import Discriminator
from src.gans.generators import Generator
from os.path import abspath, join
from random import sample
import string
import numpy as np
from src.mongo_interface import save_disc, save_gen, filter_discs, \
    filter_gens, update_disc, update_gen
from src.fitness_scores import pathogen_host_fitness, cumulative_host_fitness
...
def weights_init(m):
    """
    Performs weights initialization on the generator and discriminator.

    :param m: object on which to perform initialization
    """
...
class GANEnvironment(object):
    ...
    def hyperparameter_key(self):
        """
        Generates a trainer hyperparameters dictionary
...
def match_training_round(generator_instance, discriminator_instance,
                         ...
                         timer=None):
    """
    Master function, performing both training, partial training and matching between a generator
    and a discriminator

    :param generator_instance: generator
    :param discriminator_instance: discriminator
    ...
    :param dataloader: pythorch dataloader that supplies the real images
    :param device: device on which the training is performed
    :param latent_vector_size: latent vectors size
    :param mode: "match" | "train" | "train_g" | "train_d": respectively: match only | train both |
    train generator only | train discriminator
    :param real_label: label used for the real image
    :param fake_label: label used for the fake image
    :param training_epochs: epochs training is run for
    :param timer: StopWatch object instance used to time execution time
    :return: trace of training or matching
    """
    training_trace = []
    match_trace = []
    ...
    for epoch in range(training_epochs):
        for i, data in enumerate(dataloader, 0):
            ...
            output = discriminator_instance(fake.detach())  # flags input as non-gradientable
...
class Arena(object):
    ...
    def match(self, timer=None):
        """
        Performs a match between the generator and discriminator in the arena, given the environment

        :param timer: optional StopWatch instance for timing
        :return: match trace
        """
        ...
        update_disc(self.discriminator_instance.random_tag,
        ...
        update_gen(self.generator_instance.random_tag,
    ...
    def cross_train(self, epochs=1, gan_only=False, disc_only=False, timer=None):
        """
        Perform discriminator and/or generator training, given the generator and discriminator in the arena as well
        as the environment

        :param epochs: epochs to train for
        :param gan_only: if only generator should be trained
        :param disc_only: if discriminator only should be trained
        :param timer: optional instance of the StopWatch to time execution time
        ...
        save_disc(self.discriminator_instance.save_instance_state())

        save_gen(self.generator_instance.save_instance_state())
    ...
    def sample_images(self, annotation=''):
        """
        Samples images that can be generated by the generator in the arena

        :param annotation: optional annotation for the images that have been generated
        """
