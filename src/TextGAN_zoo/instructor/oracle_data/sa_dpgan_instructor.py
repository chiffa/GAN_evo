...
from instructor.oracle_data.instructor import SelfAttentionInstructor
...
class SADPGANInstructor(SelfAttentionInstructor):
    ...
    def adv_train_generator(self, g_step):
        ...
        for step in range(g_step):
            inp = self.oracle_data.random_batch()['input']
            if cfg.CUDA:
                inp = inp.cuda()
    ...
    def train_discriminator(self, d_step, d_epoch, phase='MLE'):
        ...
        for step in range(d_step):
            ...
            for epoch in range(d_epoch):
                # ===Train===
                self.train_dis_epoch(self.dis, pos_samples, neg_samples, self.dis_opt)

            # ===Test===
            pos_reward, neg_reward = self.eval_dis(self.dis, pos_val, neg_val)
            self.log.info('[%s-DIS] d_step %d: pos_reward = %.4f, neg_reward = %.4f,' % (
                phase, step, pos_reward.item(), neg_reward.item()))
